...
def solve_vrp_from_jobs(jobs, time_limit=360):
    ...
    # transport time to delivery
    for l in L_SET:
        g_l = TASK_DATA[l]['g_l']
        for p in P_NODES:
            d_pg = calculate_distance(p, g_l)
            model.addConstr(T_Del[l] >= T_Pick[l] + d_pg - M_local * (1 - A_P[l, p]), name=f"C7_trans_{l}_{p}")
...
def write_results_and_gantt(all_event_results, out_json='results_events.json'):
    """Write results JSON and generate Gantt PNGs.

    This function only changes plotting; it intentionally does not modify
    any model data. The layout is redesigned to clearly separate transport,
    processing (by job type), and return segments and to closely match the
    provided sample aesthetics.
    """
    ...
    for ev_idx, ev in enumerate(all_event_results, start=1):
        ev_out = {
            ...
            'total_transport_reconstructed': ev.get('total_transport_reconstructed', 0.0),
            'total_return_distance': ev.get('total_return_distance', 0.0),
            'jobs': []
        }
        for jid, js in sorted(ev.get('job_summaries', {}).items(), key=lambda x: x[0]):
            ev_out['jobs'].append(js)
        ev_out['sequences'] = {str(m): ev.get('sequence_map', {}).get(m, []) for m in M_SET}
        simple.append(ev_out)

        if not HAS_MPL:
            print("matplotlib not available: skipping gantt generation (install matplotlib to enable).")
            continue

        # Visual defaults
        mpl.rcParams.update({'font.size': 12, 'axes.titlesize': 18, 'axes.labelsize': 12})
        fig_w = 14.0
        fig_h = max(4.0, 1.25 * len(M_SET) + 2.0)
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))

        # Colors
        transport_color = '#66b3ff'
        return_color = '#f4b942'
        type_color_map = {'A': '#66c2a5', 'B': 'mediumpurple', 'C': '#ff8c42'}
        unknown_type_color = '#8c8c8c'

        # compute x axis limit (use makespan as primary reference)
        x_max = float(ev.get('makespan', 0.0))
        if x_max <= 0.0:
            for seq in ev.get('sequence_map', {}).values():
                for t in seq:
                    x_max = max(x_max, float(t.get('end_time', 0.0)))
        x_margin = max(1.0, 0.05 * max(1.0, x_max))
        ax.set_xlim(0, x_max + x_margin)

        # X major ticks every 10s with light dashed grid lines (like sample)
        ax.xaxis.set_major_locator(MultipleLocator(10))
        ax.grid(axis='x', linestyle='--', alpha=0.35)

        # AGV layout: AGV 1 at top
        agv_list = sorted(list(M_SET))
        n_agv = len(agv_list)
        row_height = 1.0
        y_positions = {m: (n_agv - 1 - i) * row_height for i, m in enumerate(agv_list)}
        y_ticks = [y_positions[m] for m in agv_list]
        y_labels = [f"AGV {m}" for m in agv_list]

        # Bar geometry (keeps processing and transport visually separated)
        transport_h = 0.6
        processing_h = 0.68
        return_h = 0.45
        transport_offset = 0.16
        processing_offset = -0.06
        return_offset = 0.02

        # Draw each AGV row
        for m in agv_list:
            y = y_positions[m]
            seq = ev.get('sequence_map', {}).get(m, [])
            prev_end = None
            for task in seq:
                pick = float(task.get('pick_time', 0.0))
                delv = float(task.get('del_time', 0.0))
                end = float(task.get('end_time', 0.0))

                # Determine lengths: prefer grid distance for transport, fallback to model time-diff
                pickup_node = task.get('pickup_node', None)
                delivery_node = task.get('delivery_node', None)
                grid_dist = None
                if pickup_node is not None and delivery_node is not None:
                    try:
                        grid_dist = float(calculate_distance(int(pickup_node), int(delivery_node)))
                    except Exception:
                        grid_dist = None
                time_diff = max(0.0, float(delv - pick))
                # Prefer the actual model time difference (del - pick) for plotting transport
                # because it matches the timeline; fall back to grid distance only when
                # time_diff == 0. This ensures transports that take time are visible.
                if time_diff > 0.0:
                    transport_len = max(time_diff, 0.5)
                elif grid_dist is not None and grid_dist > 0.0:
                    transport_len = max(grid_dist, 0.5)
                else:
                    # nothing meaningful: still draw a tiny visible bar
                    transport_len = 0.5

                proc_len = float(task.get('proc_time', max(0.0, end - delv)))
                typ = (task.get('type') or '?')
                jid = task.get('jid')

                # Transport bar (blue) - drawn above centerline. Always draw with a minimum width so it's visible.
                if transport_len > 0:
                    draw_len = max(transport_len, 0.5)
                    ax.barh(y + transport_offset, draw_len, left=pick, height=transport_h, color=transport_color, edgecolor='k', linewidth=0.7)
                    # show the label based on the more-meaningful value (prefer grid_dist when available)
                    # label shows the time-difference when available, else the grid distance
                    label_val = int(round(time_diff)) if time_diff > 0 else (int(round(grid_dist)) if (grid_dist is not None and grid_dist > 0) else 0)
                    ax.text(pick + draw_len / 2.0, y + transport_offset, f"Trans {label_val}s", va='center', ha='center', fontsize=10, fontweight='bold', color='black')

                # Processing bar (colored) - drawn below centerline
                pcolor = type_color_map.get(typ, unknown_type_color)
                if proc_len > 0:
                    ax.barh(y + processing_offset, proc_len, left=delv, height=processing_h, color=pcolor, edgecolor='k', linewidth=0.7)
                    label = f"Job{jid} {typ}({int(round(proc_len))}s) M{m}"
                    ax.text(delv + proc_len / 2.0, y + processing_offset, label, va='center', ha='center', fontsize=10, fontweight='bold', color='white')

                # Return segment between prev_end and this pick
                if prev_end is not None:
                    return_d = float(task.get('return_dist_to_pickup', 0.0))
                    if return_d > 0:
                        ax.barh(y + return_offset, return_d, left=prev_end, height=return_h, color=return_color, edgecolor='k', linewidth=0.7)
                        ax.text(prev_end + return_d / 2.0, y + return_offset, f"Return {int(round(return_d))}s", va='center', ha='center', fontsize=9, color='black')

                prev_end = end

        # Axis labels and title (two-line title, bold)
        ax.set_yticks(y_ticks)
        ax.set_yticklabels(y_labels)
        ax.set_ylim(-0.6, (n_agv - 1) * row_height + 0.6)
        ax.set_xlabel('Time (seconds)')
        ax.set_title(f"AGV Task Allocation Gantt Chart (Final Return Not Counted in Makespan) - Batch {ev_idx}\nJobs: {ev.get('n_jobs',0)}, Makespan: {ev.get('makespan',0.0):.1f}s", weight='bold', pad=12)

        # Legend (ordered)
        legend_items = [
            Patch(facecolor=transport_color, edgecolor='k', label='Transport Time'),
            Patch(facecolor=type_color_map.get('A', unknown_type_color), edgecolor='k', label='Type A Processing'),
            Patch(facecolor=type_color_map.get('B', unknown_type_color), edgecolor='k', label='Type B Processing'),
            Patch(facecolor=type_color_map.get('C', unknown_type_color), edgecolor='k', label='Type C Processing'),
            Patch(facecolor=return_color, edgecolor='k', label='Return Time (Non-last Tasks)')
        ]
        ax.legend(handles=legend_items, loc='upper right', frameon=True, bbox_to_anchor=(0.98, 0.88))

        plt.tight_layout()
        fname = f"gantt_event_{ev_idx}.png"
        try:
    ...
    # write JSON for downstream use (unchanged)
    try:
...
def main():
    ...
    write_results_and_gantt(all_event_results, out_json='results_events.json')
