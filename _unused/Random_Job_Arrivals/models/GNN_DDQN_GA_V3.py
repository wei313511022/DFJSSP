...
CONFIG = {
    ...
    'SAVE_PATH': 'gnn_ddqn_model_v3/gnn_ddqn_model_v3.pth',
...
class GeneticOptimizer:
    ...
    def nearest_base_to_station(self, station_pos: Tuple[float, float], amr_starts: List[Tuple[float, float]]) -> Tuple[float, float]:
        # In V2, we don't have explicit bases, but we can use SUPPLY locations as bases or initial positions
        # For simplicity, let's use SUPPLY_A/B/C as bases
        bases = [STATIONS["SUPPLY_A"], STATIONS["SUPPLY_B"], STATIONS["SUPPLY_C"]]
        return min(bases, key=lambda base: GLOBAL_MAP.get_true_distance(base, station_pos))

    def decode_schedule(self, individual: Individual, jobs: List[Job], amrs_state: List[AMR]) -> Tuple[Dict[int, float], float]:
        ...
        for amr in amrs_state:
            ...
            # If busy, assume it finishes at dest. If idle, at current x,y
            # For simplicity in GA, we assume start pos is current (x,y)
            current_position[aid] = (amr.x, amr.y)
            inventory[aid] = amr.inventory.copy()

        # Track station availability (simple contention model)
        ...
        for pos, job_idx in enumerate(order):
            ...
            if inventory[amr_id].get(material, 0) == 0:
                supply_pos = job.supply_pos
                dist_to_supply = GLOBAL_MAP.get_true_distance(current_position[amr_id], supply_pos)
                travel_time = dist_to_supply / CONFIG['AMR_SPEED']
            ...
            if next_job and should_return:
                ...
                # Here we approximate by finding nearest base to current station
                next_base = self.nearest_base_to_station(next_job.dest_pos, [])
                dist_return = GLOBAL_MAP.get_true_distance(job.dest_pos, next_base)
                return_time = dist_return / CONFIG['AMR_SPEED']
                
                availability[amr_id] = return_start + return_time
    ...
    def fitness(self, individual: Individual, jobs: List[Job], amrs_state: List[AMR]) -> float:
        _, total_flow = self.decode_schedule(individual, jobs, amrs_state)
        return total_flow
    ...
    def smart_load_balance_mutate(self, individual: Individual, jobs: List[Job], amrs_state: List[AMR]):
        availability, _ = self.decode_schedule(individual, jobs, amrs_state)
    ...
    def local_improve(self, individual: Individual, jobs: List[Job], amrs_state: List[AMR], max_iters: int = 100) -> Individual:
        current = Individual(order=list(individual.order), amr_assignment=list(individual.amr_assignment))
        best_score = self.fitness(current, jobs, amrs_state)
        job_count = len(current.order)
        if job_count < 2: return current
        
        availability, _ = self.decode_schedule(current, jobs, amrs_state)
        ...
        for _ in range(max_iters):
            ...
            if current.amr_assignment[job_i] == critical_amr or current.amr_assignment[job_j] == critical_amr:
                ...
                score = self.fitness(neighbor, jobs, amrs_state)
    ...
    def solve(self, waiting_jobs, amrs, env):
        ...
        for gen in range(self.generations):
            ...
            for ind in population:
                m = self.fitness(ind, waiting_jobs, amrs)
                scored.append((m, ind))
        ...
        # Local Improvement
        archive_best = self.local_improve(archive_best, waiting_jobs, amrs)
...
def main():
    ...
    for ep in range(CONFIG['NUM_EPISODES']):
        ...
        if ep % 100 == 0:
            ckpt_path = f"gnn_ddqn_model_v3/gnn_ddqn_model_v3_ep{ep}.pth"
