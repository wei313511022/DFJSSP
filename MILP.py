...
def write_results_and_gantt(all_event_results, out_dir='results'):
    ...
    MODIFIED: Output JSONL format is now sorted by 'pick_time' and contains 
    only: 'generated_at', 'amr', 'jid', 'type', 'proc_time', 'station'.
    """
    import os
    ...
    for ev_idx, ev in enumerate(all_event_results, start=1):
        ...
        try:
            ...
            # 1. 收集所有 jobs
            for m in M_SET:
                all_jobs_in_event.extend(seq_map.get(m, []))
            
            # 2. 按照 pick_time 排序 (ascending)
            #    如果 pick_time 相同，則按照 jid 排序，以確保輸出穩定性
            all_jobs_in_event.sort(key=lambda j: (j.get('pick_time', 0.0), j.get('jid', 0)))
            
            with open(event_file, 'w', encoding='utf-8') as f:
                for job in all_jobs_in_event:
                    amr = job.get('assigned_agv')
                    jid = job.get('jid')
                    type_ = job.get('type')
                    proc_time = job.get('proc_time')
                    delivery_node = job.get('delivery_node')
                    
                    if amr is None or jid is None:
                        continue
                    
                    # Find original station from delivery node
                    station = None
                    ...
                    # 3. 按照使用者要求的格式輸出
                    # generated_at 應使用 event 的 time (即 dispatch time)
                    output_record = {
                        'generated_at': float(ev.get('event_time', 0.0)),
                        'amr': int(amr),
                        'jid': int(jid),
                        'type': type_ if type_ else '?',
                        'proc_time': float(proc_time) if proc_time else 0.0,
    ...
    try:
        ...
        for ev_idx, ev in enumerate(all_event_results, start=1):
            ev_out = {
                ...
                'n_jobs': len(ev['job_summaries']), # 使用 job_summaries 的數量來確保計數準確
